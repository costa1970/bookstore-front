...
export class LivroUpdateComponent implements OnInit {
  ...
  update(): void {
    this.service.update(this.livro).subscribe((resosta) => {
      this.router.navigate([`categorias/${this.id_cat}/livros`])
  ...
  getMessage(){
    if(this.titulo.invalid) { 
      return 'O campo TITULO deve ter entre 4 e 50 caracteres';
    }
    if(this.nome_autor.invalid) { 
      return 'O campo NOME DO AUTOR deve ter entre 4 e 100 caracteres';
    }
